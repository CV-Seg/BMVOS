...
class VOS(nn.Module):
    ...
    def forward(self, feats, norm_key, full_state, object_ids):
        ...
        for k in object_ids:
            ...
            matching_score = self.matcher(init_sim, prev_sim, full_state[k])
            final_score[k] = self.decoder(feats, matching_score, full_state[k]['mask_feats'])
        return final_score
...
class BMVOS(nn.Module):
    ...
    def forward(self, x, given_masks=None, val_frame_ids=None):
        ...
        for i in range(1, L):
            ...
            norm_key = self.vos.matcher.get_norm_key(feats['s16'])
            final_score = self.vos(feats, norm_key, state, object_ids)
            predicted_seg = {k: F.softmax(final_score[k], dim=1) for k in object_ids}
            ...
            update_seg_16 = {k: F.avg_pool2d(update_seg[k], 16) for k in object_ids}
